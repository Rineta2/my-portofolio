...
const ArticleCard = ({ article, imageSize, isMain }) => {
  ...
  const slug = `/articles/${createSlug(article.categoryName)}/${createSlug(
    article.slug
  )}`;
    ...
    >
      <Link href={slug} className={styles.article__card}>
        ...
        >
          ...
          {!isMain && imageSize === 200 && (
            <p>{getTruncatedDescription(article.description, 100)}</p>
          )}
          {!isMain && imageSize === 300 && (
            <p>{getTruncatedDescription(article.description, 350)}</p>
          )}
